        ...
        {showMatches ? (
          <div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* Fallback dummy matches for demo mode */}
              {myMatches.length < 5 &&
                dummyProfiles.slice(0, 5).map((p, idx) => {
                  const dummyMatch = {
                    id: `dummy_${idx}`,
                    matched_profile: {
                      id: `dummy_profile_${idx}`,
                      display_name: p.name,
                      avatar_url: p.avatar,
                    },
                    compatibility_score: Math.floor(60 + Math.random() * 40),
                  } as any;
                  ...
                  return (
                    <div
                      key={`dummy_${idx}`}
                      className="bg-slate-800 rounded-xl p-6 cursor-pointer hover:bg-slate-700 transition-colors"
                      onClick={() => setActiveChat(dummyMatch)}
                    >
                      <div className="flex items-center gap-4 mb-4">
                        <img
                          src={p.avatar || 'https://via.placeholder.com/60'}
                          alt={p.name}
                          className="w-16 h-16 rounded-full"
                        />
                        <div>
                          <h3 className="text-xl font-semibold text-white">{p.name}</h3>
                          <p className="text-emerald-400 font-semibold">{dummyMatch.compatibility_score}% Match</p>
                        </div>
                      </div>
                      <button className="w-full py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors">
                        Start Chat
