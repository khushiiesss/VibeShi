...
import { storeAccessToken } from '../services/spotifyAuth';
...
export default function Callback() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        setStatus('Generating your profile...');
        const dummyToken = `dummy_token_${Date.now()}`;
        storeAccessToken(dummyToken);

        setStatus('Loading your music data...');
        const spotifyData = await fetchAllSpotifyData(dummyToken);

        setStatus('Creating your account...');
        const randomId = Math.random().toString(36).substring(7);
        const email = `${randomId}@ibeishi.local`;
