...
export default function ChatModal({ match, onClose }: ChatModalProps) {
  ...
  useEffect(() => {
    const isDummy = typeof match.id === 'string' && String(match.id).startsWith('dummy_');

    if (isDummy) {
      // For dummy matches use local-only messages (no DB operations)
      setMessages([]);
      return;
    }
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (isDummy) {
      // Local-only message for demo mode
      const fakeMsg: any = {
        id: `local_${Date.now()}`,
        match_id: match.id,
        sender_id: user.id,
