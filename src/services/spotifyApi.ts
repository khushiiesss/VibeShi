...
import { createDummySpotifyData, dummyProfiles } from '../lib/dummyData';
...
export const fetchUserProfile = async (accessToken: string) => {
  if (USE_DUMMY_DATA) {
    const seed = Math.floor(Math.random() * Math.max(1, dummyProfiles.length));
    return createDummySpotifyData(seed).profile;
  }
...
export const fetchTopArtists = async (accessToken: string): Promise<SpotifyArtist[]> => {
  if (USE_DUMMY_DATA) {
    const seed = Math.floor(Math.random() * Math.max(1, dummyProfiles.length));
    return createDummySpotifyData(seed).topArtists;
  }
...
export const fetchTopTracks = async (accessToken: string): Promise<SpotifyTrack[]> => {
  if (USE_DUMMY_DATA) {
    const seed = Math.floor(Math.random() * Math.max(1, dummyProfiles.length));
    return createDummySpotifyData(seed).topTracks;
  }
...
export const fetchUserPlaylists = async (accessToken: string): Promise<SpotifyPlaylist[]> => {
  if (USE_DUMMY_DATA) {
    const seed = Math.floor(Math.random() * Math.max(1, dummyProfiles.length));
    return createDummySpotifyData(seed).playlists;
  }
...
export const fetchAudioFeatures = async (accessToken: string, trackIds: string[]): Promise<AudioFeatures> => {
  if (USE_DUMMY_DATA) {
    const seed = Math.floor(Math.random() * Math.max(1, dummyProfiles.length));
    return createDummySpotifyData(seed).audioFeatures;
  }
...
export const fetchAllSpotifyData = async (accessToken: string) => {
  if (USE_DUMMY_DATA) {
    const seed = Math.floor(Math.random() * Math.max(1, dummyProfiles.length));
    const data = createDummySpotifyData(seed);
    return {
